...
$requeteM = "SELECT count(*) FROM panier WHERE IDutilisateur='".$_SESSION['ID']."';";
...
if( ($db_found) && ($countM > 0) ){
    $sql3="SELECT IDproduit,IDpanier FROM panier WHERE IDutilisateur='".$_SESSION['ID']."';";
    ...
    while ($data = mysqli_fetch_assoc($result3)) {
        ...
        $sqlter = "SELECT IDproduit,Nom,Description,Prix,Categorie,Qualite,Defaut,OffreSpeciale,IDutilisateur FROM produit WHERE IDproduit='".$data['IDproduit']."';";
        $exec_requete = mysqli_query($db,$sqlter);
        $reponseter      = mysqli_fetch_assoc($exec_requete);
        
        
        $prestigeMusee[$j] = array($data['IDpanier'], $reponseter['Nom'], $reponseter['Description'], $reponseter['Prix'], $reponseter['Categorie'],$reponseter['Qualite'],$reponseter['Defaut'], $reponsebis['Lien']);
        $j++;
    }
}
...
<html>
    ...
    <body>
        ...
        <!--ZONE POUR CODER-->
        <div class="container" >
                    <h2 class="title">Mon Panier</h2><br>
                    <?php
                    for($i=0;$i<$countM;$i++)
                    {
                         echo "<div class=\"produit$\">
                                ...
                                <form action=\"Delpanier.php\" method=\"post\" >
                                    <button type=\"submit\"  name=\"Supprimer\" value=\"".$prestigeMusee[$i][0]."\">Supprimer</button>
                                </form>
                            </div>";
                    }
                    ?>         
