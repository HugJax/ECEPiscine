...
//BLINDER POUR QUE L'ACHETEUR NE PUISSE FAIRE QU'UNE SEULE ET UNIQUE ENCHERE
    if($IDacheteur)             //Si la session est ouverte
    {
        ...
        {  
            ...
            //CES VARIABLES DOIVENT ETRE MODIFIEES POUR L'INTEGRATION
            //$Prix = 12;
            $IDproduit = 1;
            $IDvendeur = 3;
            
            //On recherche le prix max dans la table 
            $prixMax = "SELECT PrixMax FROM enchere WHERE IDproduit='".$IDproduit."' ";
            
            $exec_requete = mysqli_query($db_handle,$prixMax);
            $reponse      = mysqli_fetch_array($exec_requete);
            $prix = $reponse['PrixMax'];
            
            if($enchereMax > $prix)
            {
                //On insère la proposition d'achat dans la table négociation
                $requete1 = "UPDATE enchere SET PrixMax='".$enchereMax."' WHERE IDproduit = '".$IDproduit."' ";
                $db_handle->query($requete1);           //On effectue la requête d'insertion précédente
                echo "Merci pour votre offre";
            }
            else 
            {
                echo "Votre offre est malheureusement trop basse et ne sera pas prise en compte";
            }
