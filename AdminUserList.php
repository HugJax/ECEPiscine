<?php
    ...
    if($count!=0) {
        ...
        while($reponse = mysqli_fetch_assoc($exec_requete)) {
            $listuser[$i] = array($reponse['IDutilisateur'], $reponse['Nom'], $reponse['Prenom'], $reponse['Email'], $reponse['Telephone'], $reponse['Photo']);
            $i++;
        }
    }
?>
...
<html>
    ...
    <body>
        ...
        <?php
            for($x=0; $x<=($count-1); $x++) {
                echo '<div id="formContent" style="float:right;padding:30px;max-width:none;width:80%;margin-right:10%;margin-top:20px;margin-bottom:20px;height:auto;"><table><tr><td width=250px rowspan=2><img class="media-object dp img-circle" src='.$listuser[$x][5].' style="width: 180px;height:180px;"></td><td width=300px><h4>'.utf8_encode($listuser[$x][1]).' '.utf8_encode($listuser[$x][2]).'</h4></td><td width=400px>'.utf8_encode($listuser[$x][3]).'</td><td width=220px align=left>+33 '.utf8_encode($listuser[$x][4]).'</td><td align=right width=450px><form action="SupUser.php" method="post"><button name="sup" type="submit" class="but" value="'.$listuser[$x][0].'">SUPPRIMER LE COMPTE</button></td></tr></table></div>';
            }
        ?>
